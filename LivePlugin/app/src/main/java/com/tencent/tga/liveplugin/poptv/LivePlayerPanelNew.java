 ...
 */
public class LivePlayerPanelNew {
...
//    private FloatPanel floatPanel;
    ...
    public LivePlayerPanelNew(Activity activity, Resources resources, String content) {
        mActivity = activity;
    ...
    public void launch() {
        try {
            ...
            mRootView = (ViewGroup) DLPluginLayoutInflater.getInstance(mActivity).inflate(xmlResourceParser, null);

            if (mRootView == null)
                return;

            if (((ViewGroup)mActivity.getWindow().getDecorView()).indexOfChild(mRootView) == -1) {
                ((ViewGroup)mActivity.getWindow().getDecorView()).addView(mRootView);
            }
    ...
    private void init() {
        ...
        Plugin.getInstance().mLivePanel = this;
...
//                floatPanel.onDialogExit(true);
    ...
    public void finish() {
        try {
            if (((ViewGroup)mActivity.getWindow().getDecorView()).indexOfChild(mRootView) != -1) {
                ((ViewGroup)mActivity.getWindow().getDecorView()).removeView(mRootView);
            }
            exitRelease();
            releaseView();
            DLCallBackManager.setPluginCallback(null);
            Plugin.getInstance().mLivePanel = null;
        } catch (Exception E) {
            E.printStackTrace();
        }
    }
